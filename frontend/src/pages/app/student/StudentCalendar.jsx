...
export default function StudentCalendar() {
  ...
  const save = () => {
    ...
    alert(t("app.student.calendar.saved"));
  };

  return (
    <div className="card">
      <h3>{t("app.student.calendar.title")}</h3>
      <p style={{margin:"0 0 8px", opacity:.9}} dangerouslySetInnerHTML={{__html: t("app.student.calendar.descriptionStudent")}}>
      </p>
      {busyTimes.length > 0 && (
        <p style={{display:"block", margin:"4px 0 8px", color:"#ef4444", opacity:.9}}>
          {t("app.student.calendar.busyTimesNote")}
        </p>
      )}
      {loading && <p style={{margin:"0 0 8px", opacity:.7}}>{t("app.student.calendar.loading")}</p>}
      <CalendarPro role="student" locale={i18n.language} events={allEvents} onChange={setItems} />
      <div style={{display:"flex", justifyContent:"flex-end", marginTop:10}}>
        <button className="btn primary" onClick={save}>{t("app.student.calendar.save")}</button>
      </div>
    </div>
  );
}
